...
st1 = 1e-04;
% p_ini_v(8) = p_ini_v(8) + st1;
[sole.coor,dPFree_dp] = deformation_moveDiri(sole,p_ini_v,spl,param_sopt.move_dirichlet); % Moving Dirichlet
lp = length(p_ini_v);
% st1 = 1e-09;
% sole.coor(2,1) = sole.coor(2,1) + st1;
sole.stiffness();
sole.stiffnessSurface();
sole.derStiff(lp,dPFree_dp);
% a = load('Pg.mat');
% Pg = a.Pg();
...
%%% Test Gradient algo with given position and orientation
displ = [0.00000262899049116;-0.00000406461222954;-0.0001155602271939];
%displ = [0.000323007608832;-0.00409658617771;0.002179953123297];
angleact = [-0.126916077961588;-0.118047287866647;-0.000913850459974];
Pg = [];
FcFreeSurf = [];
D = 3;
...
%     displ = displ_tot(:,1);
    ...
    %dF_dPfree = zeros(3*length(ind_cont),3*sole.nTot);
    ...
    [Theta_mat,der_HCcH,der_HCcHF,Hc] = gradient_simu_Theta_contact(ind_cont,der_Cc_dp,ind_slip,Fc_mc3,R,Rini,lp,dPFreec_dp,contAngle,[]);
    inv_A = inv(A);
    inv_A_theta = inv_A * Theta_mat;
    dF_dPfree = inv_A_theta(1:3*length(ind_cont),:);
    %Fc_mc3 = testderFc_Pfree_i(sole,coorini);
    %2     5     6    23    24    75    86    91
    % 86    91
    f = @(x) testderFc_Pfree_i(sole,x);
    J = diff5points(f,[3,1],1e-04,coorini);
    (dF_dPfree(:,7)-J)./dF_dPfree(:,7)   
    %[dF_dPfree(:,1) J{1}]
%     for j=1:sole.nFree
%         contact_surf = j;
%         contact = find(contact_surf==ind_cont,1);  
%         if ~isempty(find(contact_surf==ind_cont,1))
%             Theta_mat = gradient_simu_Theta_contact(contact,contact_surf,ind_cont,der_C_cc_dPfree,ind_slip,Fc_mc3,R,Rini);
%             inv_A_theta = inv_A * Theta_mat;
%             dF_dPfree(:,(3*j)-2:(3*j)) = inv_A_theta(1:3*length(ind_cont),:);
%         else
%             contact = 0;
%             Theta_mat = gradient_simu_Theta_contact(contact,contact_surf,ind_cont,der_C_cc_dPfree,ind_slip,Fc_mc3,R,Rini);     
%             inv_A_theta = inv_A * Theta_mat;
...
%         contact = find(contact_surf==ind_cont,1);  
%         if ~isempty(find(contact_surf==ind_cont,1))
%             Theta_mat(:,(3*j)-2:(3*j)) = gradient_simu_Theta_contact(contact,contact_surf,ind_cont,der_C_cc_dPfree,ind_slip,Fc_mc3,displ_ini,psi_ini,angleact);
% %             inv_A_theta = inv_A * Theta_mat;
% %             dF_dPfree(:,(3*j)-2:(3*j)) = inv_A_theta(1:3*length(ind_cont),:);
%         else
%             contact = 0;
